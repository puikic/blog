...
type Mystruct struct {
	...
	CreatTime  time.Time `form:"create_time" binding:"required,before_tody" time_format:"2006-01-02" time_utc:"8"`
	int
	test string
}
...
func PrintFieldInfo(object any) {
	...
	for i := 0; i < fieldNum; i++ {
		field := tp.Field(i)
		fmt.Printf("%d %s offset %d anoymous %t type %s exported %t gorm tag=%s json tag=%s\n", i,
			field.Name,
