...
type JwtHeader struct {
	Algo string `json:"alg"`  //哈希算法，默认为HMAC,SHA256(写为HS256)
	Type string `json:"type"` //令牌(token)类型，统一写为JWT
}
...
type JwtPayload struct {
	...
	NotBefore   int64          `json:"nbf"` //在此之前不可用
...
func GenJWT(header JwtHeader, payload JwtPayload, secret string) (string, error) {
	// 参数secret是HMAC的密钥（key），用于对消息进行加密和验证
	...
	//基于sha256的哈希认证算法。任意长度的字符串,经过sha256之后长度都变成了256bits
...
func VerifyJwt(token, secret string) (*JwtHeader, *JwtPayload, error) {
	...
	if err = json.Unmarshal(part1, &header); err != nil {
		return nil, nil, fmt.Errorf("header json 反解失败")
	}
	if err = json.Unmarshal(part2, &payload); err != nil {
		return nil, nil, fmt.Errorf("payload json 反解失败")
	}
